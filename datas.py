...
# book = pd.concat([pd.DataFrame(datas['book']),pd.DataFrame(datas['lang'])], axis=1)
# print(book)
...
# l_url = datas['url']
# l_url = pd.DataFrame(l_url)

# url = 'http://images.amazon.com/images/P/0671027360.01.LZZZZZZZ.jpg'
# headers = {
...
# imag=convert_to_bytes(pi, (500, 200), fill=False)

# app = QApplication([])

# image = QImage()
# image.loadFromData(imag)
# image_label = QLabel()
...
# for row in l_url.iterrows():
#     try:
#         image = QImage()
#         image.loadFromData(requests.get(row[1].values[0]).content)
#         image_label = QLabel()
#         image_label.setPixmap(QPixmap(image))
#         image_label.show()
